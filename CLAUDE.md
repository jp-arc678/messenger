...
- **D34 — รูปแบบวันที่บนหน้าจอ (ต่อจาก D19):** ✅ ทุกหน้าต้องเป็น **dd/MM/yyyy** ทั้งข้อความแสดงผล
  และ**ช่องกรอก** · ข้อความแสดงผลใช้ `.ToString("dd/MM/yyyy")` ตรง ๆ อยู่แล้ว
  · ส่วนช่องกรอก `<input type="date">` เบราว์เซอร์เป็นคนเลือกรูปแบบที่วาดตามภาษาที่ตั้งไว้
  (เครื่องที่ตั้งอังกฤษ US จะได้ mm/dd/yyyy) **สั่งจากฝั่งเว็บไม่ได้ทั้ง CSS และ attribute**
  → แก้ด้วย `Scripts\date-field.js` ที่ยกช่อง `type="date"` ทุกช่องเป็นช่องข้อความ dd/MM/yyyy
  แล้วซ้อนช่องตัวจริงแบบโปร่งใสไว้ใต้ปุ่มปฏิทิน (ยังเรียกปฏิทินของเครื่องได้ทั้ง desktop/มือถือ
  ตาม UAT 8.5) · **ค่าที่ส่ง server ยังเป็น ISO `yyyy-MM-dd` เหมือนเดิม** ฝั่ง C# ไม่ต้องแก้
  · สคริปต์โหลดที่ `_Layout` จึงมีผลทุกหน้าอัตโนมัติ **ช่องวันที่ใหม่ไม่ต้องทำอะไรเพิ่ม**
- **D35 — cache ของไฟล์ static (ต่อจาก D34):** ✅ อ้างไฟล์ css/js ด้วย `@Url.Asset("~/...")`
  (`src\Web\Helpers\AssetUrl.cs`) ที่ต่อท้าย `?v={เวลาแก้ไขไฟล์}` ให้อัตโนมัติ **ห้ามใช้
  `@Url.Content` กับ css/js อีก**
  เหตุผล: IIS ส่งไฟล์ static มาโดยไม่มี `Cache-Control` เบราว์เซอร์จึงใช้ heuristic caching
  ของตัวเองแล้วหยิบไฟล์เก่ามาใช้โดยไม่ถาม server · อาการที่เจอจริงคือเครื่องที่ยังถือ `site.css`
  เวอร์ชันก่อน D34 จะไม่มีกฎ `.date-field-picker > input[type="date"] { opacity: 0 }`
  ทำให้ `<input type="date">` ตัวจริงโผล่มาให้เห็นและวาดเป็น **mm/dd/yyyy** — คือย้อนกลับไป
  เป็นอาการเดิมที่ D34 ตั้งใจแก้ ทั้งที่โค้ดถูกแล้ว · แก้แบบนี้แล้วผู้ใช้ไม่ต้อง Ctrl+F5 อีก
  และตอน dev แก้ css/js แล้ว refresh เห็นผลทันทีโดยไม่ต้อง restart แอป
- **D36 — ประเภทงานในหน้ารายการใบแจ้งงาน:** ✅ หน้า `/Requests` แสดงคอลัมน์ **ประเภทงาน**
  เป็น `ชื่อประเภท : รายละเอียด` ต่อบรรทัด (เช่น `ต่อภาษี : รถ Toyota vigo`) ใบที่มีหลายประเภท
  แสดงครบทุกบรรทัด · ใบที่ไม่ได้กรอกรายละเอียดแสดงแค่ชื่อประเภท
  · `spDeliveryRequestList` จึงคืน **result set 2 ชุด** (ใบงาน + ประเภทงานของทั้งชุด) แล้ว
  repository ประกอบเข้าด้วยกัน — ยังวิ่ง DB รอบเดียว **ไม่ใช่ N+1**
  · ใช้แถวจริงแทนการต่อสตริงใน view เพราะ `DetailText` เป็นข้อความอิสระของผู้ใช้
  (ผู้ใช้พิมพ์จุลภาคเองแล้วการแยกกลับจะพัง) — `JobTypeCodes` ใน `vwDeliveryRequest` จึงยังคง
  มีแต่ "รหัส" ประเภทงานตามเดิม ไม่แตะต้อง
- **D37 — สิทธิ์ "ดู" ใบงานกว้างกว่าสิทธิ์ "แก้" (ต่อจาก D17):** ✅ **ดู**ได้ = Admin/Messenger ทั้งสาขา
  · **ผู้แจ้ง** · **คนที่กดสร้าง** — ส่วน**แก้/ยกเลิก**ยังเป็นของ**ผู้แจ้งคนเดียว**ตาม D17 ไม่เปลี่ยน
  เหตุผล (UAT รอบ 1 ข้อ 2.8): เดิม "ใบของฉัน" ถูกนิยามด้วย `RequesterEmpCode` อย่างเดียว
  พอแจ้งแทนคนอื่น คนกรอกจะถูกเด้ง **"คุณไม่มีสิทธิ์ดูใบแจ้งงานนี้"** ทันทีที่บันทึกเสร็จ
  (เพราะ `Create` redirect ไปหน้ารายละเอียด) และใบนั้นก็ไม่โผล่ในรายการของตัวเองอีกเลย
  → **ฟีเจอร์แจ้งแทนคนอื่นใช้งานไม่ได้จริง**
  · นิยามอยู่ที่ `RequestAccess.CanSee()` ที่เดียว ใช้ร่วมกันทั้งใบงาน/รูป/ประวัติสถานะ
  · ตัวกรองรายการใช้ `RequestListFilter.VisibleToEmpCode` → SQL เทียบ
  `RequesterEmpCode = @x OR CreatedBy = @x` · **BR-6 ยังมาก่อนเสมอ** (ต่างสาขาไม่เห็นแม้ `CreatedBy` ตรง)
- **D38 — สีป้ายสถานะ:** ✅ ทุกสถานะมีสีต่างกัน รวมไว้ที่ `Web\Helpers\StatusBadge.cs` ที่เดียว
  (รับแจ้ง=เทา · กำลังส่ง=น้ำเงิน · พักการส่ง=เหลือง · เสร็จงานแล้ว=เขียว · ยกเลิก=แดง)
  สีล้อกับปุ่มการกระทำใน `StatusActionsViewModel.ButtonClass` · **หน้าใหม่ให้เรียก
  `StatusBadge.CssClass(status)` เสมอ ห้ามใส่ `text-bg-*` เอง** ไม่งั้นสีจะเริ่มไม่ตรงกันข้ามหน้า
- **D39 — ต้องรัน .sql ด้วย `-f 65001` เท่านั้น:** ✅ ใช้ `pwsh -File tools\Verify.ps1`
  (หรือ `sqlcmd -f 65001 -i ...`) **ห้ามรัน `sqlcmd -i` เปล่า ๆ**
  เหตุผล: ไฟล์ .sql เป็น UTF-8 แต่ sqlcmd อ่านด้วย codepage ของ Windows (874) ถ้าไม่บอก
  ทำให้ข้อความไทยใน literal เพี้ยนตั้งแต่ตอน deploy — อาการที่เจอจริงคือ `N'สร้างใบแจ้งงาน'`
  ใน `spDeliveryRequestCreate` กลายเป็น `N'เธชเธฃเนเธฒเธ...'` แล้วไปโผล่ใน**ประวัติสถานะของทุกใบที่สร้างหลังจากนั้น**
  · โค้ดต้นฉบับถูกอยู่แล้ว ผิดที่คำสั่ง deploy — เป็นบั๊กที่หายากเพราะ build/test ผ่านหมด
