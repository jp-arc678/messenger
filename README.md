...
| Phase 3 — Photo (รูปยืนยัน BR-3 + เงื่อนไขปิดงาน BR-4) | ✅ เสร็จ |
| Phase 4 — Notification (อีเมลแจ้งผู้แจ้งตอนปิดงาน BR-5) | ✅ เสร็จ |
| Phase 5 — Reporting / Dashboard (สรุป + export Excel) | ✅ เสร็จ |
| Phase 6 — Branch isolation + hardening + UAT | ✅ เสร็จ — รอ approval |

> 🎉 ครบทั้ง 6 เฟสตามแผนใน [CLAUDE.md §9](CLAUDE.md) แล้ว
> ก่อนขึ้นใช้จริงให้ไล่ [UAT Checklist](docs/UAT-Checklist.md) ให้ครบก่อน
...
## Phase 4 — อีเมลแจ้งผู้แจ้งตอนปิดงาน (BR-5)

ปิดงาน (→ `Completed`) แล้วระบบส่งอีเมลอัตโนมัติ · สถานะอื่นไม่ส่ง

| เรื่อง | กฎ |
|---|---|
| ผู้รับ | `To` = ผู้แจ้ง · `Cc` = คนที่กดสร้างใบงาน เฉพาะเมื่อแจ้งแทนคนอื่น (D27) |
| ผู้แจ้งไม่มีอีเมล | ไม่ส่ง แต่**ปิดงานสำเร็จ** พร้อมคำเตือนสีเหลืองบนหน้าจอ |
| ส่งไม่ออก (SMTP ล่ม) | **ไม่ย้อนสถานะ** งานยังปิดอยู่ + คำเตือน (D26) |

### ตอนนี้เมลไปไหน

ยังไม่มี SMTP ของบริษัท (D5) ระบบจึงเขียนไฟล์ **`.eml`** ลง `src\Web\App_Data\Mail\`
เปิดด้วย Outlook / โปรแกรมอ่านเมลได้เลย เนื้อความครบเหมือนของจริง

**เมื่อได้ SMTP จริง** แก้แค่ [Web.config](src/Web/Web.config) ไม่ต้องแตะโค้ด:

```xml
<add key="EmailPickupDirectory" value="none" />   <!-- เดิมเป็นค่าว่าง -->
...
<system.net>
  <mailSettings>
    <smtp deliveryMethod="Network" from="ที่อยู่ผู้ส่งจริง">
      <network host="smtp ของบริษัท" port="25" />
    </smtp>
  </mailSettings>
</system.net>
```
...
แก้ [App_Data/EmailTemplates/RequestCompleted.html](src/Web/App_Data/EmailTemplates/RequestCompleted.html)
แล้วมีผลทันทีโดยไม่ต้อง build ใหม่

- บรรทัดแรก `Subject: ...` คือหัวเรื่อง (ไม่มีบรรทัดนี้ = ใช้หัวเรื่องมาตรฐาน)
- ตัวแปรเขียนแบบ `{{ReqNo}}` `{{ContactName}}` `{{SendDate}}` `{{MessengerName}}` ฯลฯ (ดูรายการในไฟล์)
- ค่าทุกตัวถูก HTML-encode ให้อัตโนมัติ — ข้อความจากผู้ใช้แทรก tag เข้ามาไม่ได้
- ลบไฟล์ทิ้ง = กลับไปใช้ template สำรองที่ฝังในโค้ด

### ลองด้วยมือ

1. ลบไฟล์เก่าใน `src\Web\App_Data\Mail\` → ปิดงานสัก 1 ใบ → ต้องมีไฟล์ `.eml` โผล่ 1 ไฟล์
2. เปิดไฟล์ดู: `To` เป็นอีเมลผู้แจ้ง · ถ้าแจ้งแทนคนอื่นจะมี `Cc` เป็นคนกรอก
3. ลองตั้ง `EmailPickupDirectory` เป็น path ที่เขียนไม่ได้ → ปิดงานยังสำเร็จ แต่ขึ้นคำเตือนสีเหลือง

---

## Phase 5 — รายงานสรุป + export Excel

เมนู **รายงาน** (`/Reports`) เปิดให้ทุก role — ขอบเขตข้อมูลถูกจำกัดที่ service ตาม §5:
Admin/Messenger เห็นทั้งสาขา · User เห็นเฉพาะใบที่ตัวเองเป็นผู้แจ้ง

- เลือก**ช่วงวันที่ส่ง**ได้ (ค่าเริ่มต้น = วันนี้) พร้อมปุ่มลัด วันนี้ / สัปดาห์นี้ / เดือนนี้ (D29)
- การ์ดสรุปครบ 5 สถานะ · ตารางแยกตาม **เจ้าหน้าที่ Messenger** · ตารางแยก **รายวัน**
  (ออกครบทุกวันในช่วง วันที่ไม่มีงานแสดง 0 เพื่อให้เห็นวันที่เงียบผิดปกติ)
- แถวรายวันมีลิงก์ **เปิดคิว** ไปหน้าคิวงานของวันนั้นได้เลย

> รายงานยึด **วันที่ส่ง** เป็นแกน ไม่ใช่วันที่แจ้ง เพราะ BR-1 ทำให้งานที่แจ้งบ่ายวันศุกร์
> ไปตกวันจันทร์ — วันที่ Messenger ออกวิ่งจริงคือวันที่มีความหมายกับรายงาน

### Export Excel (D30, D31)

ปุ่ม **ดาวน์โหลด Excel** ได้ไฟล์ `Messenger-Report-{สาขา}-{ช่วงวันที่}.xlsx` มี 2 ชีต:

| ชีต | เนื้อหา |
|---|---|
| รายการใบงาน | 1 แถว = 1 ใบงาน (13 คอลัมน์) จัดเป็นตาราง Excel พร้อม filter — เอาไป pivot ต่อได้ |
| สรุป | ตัวเลขชุดเดียวกับที่เห็นบนหน้าจอ |

ใช้ **ClosedXML** (dependency ใหม่ตัวเดียวของเฟสนี้ อนุมัติแล้วตาม §10 ข้อ 8) วันที่เขียนเป็น
ค่า date จริงจึงเรียง/กรองใน Excel ได้ · เบอร์โทรบังคับเป็นข้อความ Excel จึงไม่กินเลข 0 ข้างหน้า

> ⚠️ ClosedXML เป็นไลบรารีแบบ netstandard2.0 — [Web.config](src/Web/Web.config) จึงต้องมี
> `<add assembly="netstandard, ..." />` ใน `<compilation>` และ binding redirect ของ
> `System.Buffers` / `System.Memory` / `System.Numerics.Vectors` / `System.Runtime.CompilerServices.Unsafe`
> **ห้ามลบ** ไม่งั้นทุกหน้าเว็บจะพังด้วย `CS0012` ตอน runtime

### ลองด้วยมือ

1. login `10003` (Messenger SDC) → เมนู **รายงาน** → กด **เดือนนี้** → ดูตัวเลขแยกวัน
2. กด **ดาวน์โหลด Excel** → เปิดไฟล์ ต้องได้ 2 ชีต ภาษาไทยไม่เพี้ยน
3. login `10004` (User SDC) → หน้ารายงานต้องขึ้นว่า "เฉพาะใบที่คุณเป็นผู้แจ้ง" และตัวเลขน้อยลง
4. login `20003` (Messenger SBK) → ต้องไม่เห็นใบงานของ SDC เลย

---

## Phase 6 — การแยกข้อมูลตามสาขา + hardening

### ตรวจ BR-6 แบบยิงตรง ไม่ผ่านหน้าจอ

```powershell
pwsh -File tools\Test-BranchIsolation.ps1     # ต้องผ่านครบ 32 ข้อ
```

สคริปต์นี้ login เป็นผู้ใช้ 5 คน (2 สาขา 3 ระดับสิทธิ์) แล้ว **ยิง POST/GET ตรงไปที่ action**
พร้อม id ของสาขาอื่น เหมือนคนที่แก้ URL เอง — เพราะการที่หน้าจอซ่อนปุ่มให้ ไม่นับว่าปลอดภัย

ครอบคลุม: อ่านข้ามสาขา (รายละเอียด/แก้ไข/รายการ/คิว/รายงาน) · เขียนข้ามสาขา (เปลี่ยนสถานะทั้ง 5 แบบ /
ยืนยันรับของ / จัดลำดับคิว / ลบรูป / แก้ใบงาน) · สิทธิ์ตาม role ในสาขาเดียวกัน · CSRF · เข้าถึงโดยไม่ login ·
security header

### สิทธิ์และสาขาอ่านใหม่จาก DB ทุก request

เดิมข้อมูลผู้ใช้ถูกอ่านจาก cookie ที่อยู่ได้ 8 ชั่วโมง ซึ่งแปลว่า**คนที่ถูกย้ายสาขายังเห็นข้อมูลสาขาเดิม
ได้ทั้งวัน** และคนที่เพิ่งถูกลดสิทธิ์ก็ยังใช้สิทธิ์เดิมได้ — ช่องโหว่เชิงเวลาของ BR-6

ตอนนี้ [RefreshUserContextAttribute](src/Web/Security/RefreshUserContextAttribute.cs) อ่านสาขา/สิทธิ์
ใหม่จาก DB ทุก request (1 query ต่อ request) และถ้าพนักงานถูกปิดการใช้งาน/สาขาถูกปิด จะถูกไล่ออกจาก
ระบบทันทีพร้อมข้อความบอกเหตุผล

### อื่น ๆ ในเฟสนี้

- หน้า error และ 404 ของระบบ (ไม่หลุด stack trace ให้ผู้ใช้เห็น)
- security header: `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` ·
  ไม่ส่ง `X-Powered-By` / `X-AspNet-Version` · cookie เป็น httpOnly
- [UAT Checklist](docs/UAT-Checklist.md) — 60+ ข้อครอบคลุมทุกเฟส พร้อมข้อจำกัดที่ทราบแล้ว
  และรายการที่ต้องทำก่อนขึ้น production
